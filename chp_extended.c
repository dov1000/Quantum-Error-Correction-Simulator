...

{
	...
	long n;         // # of qubits
	long a; 	// # of ancilla qubits. This was added later. MGA
...
void decouple_data_and_ancilla(struct QState *q)

// New function: Used to "decouple" the data and ancillary
// stabilizers and destabilizers from each other, so that
// after a measurement in the ancillary qubits is done, the
// disposal of the ancillae destabs/stabs won't change the
// state of the data qubits.   MGA 2012

{
	...
	for (i = 2*q->n - 1 ; i > 2*q->n - 1 - q->a ; i--)
	{
	 ...
	 {
	  //printf("\nj = %ld", j);
	  //printf("\nvalue = %lu", q->z[j][i5]&pw);
	  if (q->z[j][i5]&pw)
	  {
	   rowmult(q, j, i);
	   rowmult(q, i-q->n, j-q->n);
	  }
	 }
...
void initstae_(struct QState *q, long n, char *s, long a)
...
void initstae_from_file_(struct QState *q, long n, char *fn, long a)

// Initialize state q to have n qubits, and input specified by file fn, which has the same format as the output file of printstate_file

{
	...
	while (!feof(fp))
	{
	  ...
	  while (c!='\n')
	  {
	    //printf("\tj = %ld \n", j);
	    fscanf(fp, "%c", &c);
	    if ((c=='X') || (c=='x')) q->x[i][j>>5] += q->pw[j&31];
	    else if ((c=='Z') || (c=='z')) q->z[i][j>>5] += q->pw[j&31];
	    else if ((c=='Y') || (c=='y'))
	    { 
	      q->x[i][j>>5] += q->pw[j&31];
              q->z[i][j>>5] += q->pw[j&31];
...
void initstae_from_stab_(struct QState *q, long n, char *s, long a)

// New function: The same as initstae_ but now s is a character string 
// containing all the destabilizers and stabilizers.  For example, 
// if the initial state is |000>, then s would be '+XII+IXI+IIX+ZII+IZI+IIZ'
// MGA 2012

{
	...
	for (i = 0; i < 2*q->n; i++)
	{
		if (s[i*(q->n + 1)] == '-')  q->r[i] = 2;
		for (j = 1; j < q->n + 1; j++)
		{
			if (s[i*(q->n + 1) + j] == 'X') 
				q->x[i][(j-1)>>5] += q->pw[(j-1)&31];
			else if (s[i*(q->n + 1) + j] == 'Z')
				q->z[i][(j-1)>>5] += q->pw[(j-1)&31];
			else if (s[i*(q->n + 1) + j] == 'Y')
			{
				q->x[i][(j-1)>>5] += q->pw[(j-1)&31];
				q->z[i][(j-1)>>5] += q->pw[(j-1)&31];
			}
...
void readprog(struct QProg *h, char *fn, char *params, long n)
...
void readprog_string(struct QProg *h, char *gates, char *params, long n, long T)

{
        // read the circuit from a string instead of a chp file.
        // The whole circuit is expressed as a single string and
        // each gate should be separated by an 'n'.
        // For example, for the preparation and measurement of a
        // two-qubit cat state, we would write:
        // 'P_0nP_1nC_0_1nM_0n' 
        // MGA 2015

        long t;
        long l;
	long l1;                // length of qubit 1 (number of digits)
        long l2;                // length of qubut 2 (number of digits)
        char c;
        long num;
        long p;
        long q1;                // qubit 1
        long q2;                // qubit 2 (target if gate is CNOT)

        long line_index;        // the line index
        long len_line;          // the length of the current line
        long t_line;
        char c_line;
        long line_first_i;      // the index of the first character in the current line
        ...
        line_first_i = 0;
        for (line_index = 0; line_index < T; line_index++)
        {
          ...
          if ((gates[line_first_i]=='c')||(gates[line_first_i]=='C'))  h->a[line_index] = CNOT;
          if ((gates[line_first_i]=='h')||(gates[line_first_i]=='H'))  h->a[line_index] = HADAMARD;
          if ((gates[line_first_i]=='p')||(gates[line_first_i]=='P'))  h->a[line_index] = PHASE;
          if ((gates[line_first_i]=='m')||(gates[line_first_i]=='M'))  h->a[line_index] = MEASURE;

          //printf("Gate = %c \n", gates[line_first_i]);


          // count how many digits first qubit has
          ...
          // convert first qubit from char string to long


          q1 = 0;
          p = 1;
          for (t = line_first_i + 1 + l1; t > line_first_i + 1; t--)
          {
            num = p*(gates[t] - '0');
            q1 += num;
            p *= 10;
          ...
          // if gate is CNOT, count how many digits second qubit has

          if (h->a[line_index]==CNOT)
          {
            t = 3 + line_first_i + l1;
            l2 = 0;
            c = gates[t];
            while (c!='n')    // each line has to end with 'n'.
            {
              t++;
              l2++;
              c = gates[t];
            ...
            // convert second qubit from char string to long

            q2 = 0;
            p = 1;
            for (t = line_first_i + 2 + l1 + l2; t > line_first_i + 2 + l1; t--)
            {
              num = p*(gates[t] - '0');
              q2 += num;
              p *= 10;
...

{
	...
	int read_circuit_line = 0;  // whether to read circuit from string (terminal)
	// To indicate CHP that you want to use a string to transmit the circuit
	// (instead of the usual input file), the first character in params (after
	// the hyphen) needs to be an 'l'.	
	int num_in = 0; // number of inputs
	...
	{
	  param = 1;
	  if (argv[1][1]=='l')  read_circuit_line = 1;
	}
	...
	l = strlen(argv[argc-2]);	// total number of qubits
	p = 1;
	for (i = l-1; i > -1; i--)
	{
  	 long num = p*(argv[argc-2][i] - '0');
	 n += num;
	 p *= 10;
	} 
	...
	l = strlen(argv[argc-1]);	// total number of gates
	p = 1;
	for (i = l-1; i > -1; i--)
	{
  	 long num = p*(argv[argc-1][i] - '0');
	 T += num;
	 p *= 10;
	} 
	...
	if (param)
	{
	  if (read_circuit_line) 
	  {
	    readprog_string(h,argv[2],argv[1],n,T);  
	  }
	  else readprog(h,argv[2],argv[1],n);
	}
	else 
	{
	...
	if (argc==(7+param))   	// if instates or stabilizers are specified  
	{
	  ...
	  {
	    initstae_from_file_(q,h->n,argv[2+param],a);  
	  }
	  else
	  {	
	    // if the length of the stab input is the same as the number of qubits,
	    // it means the user is specifying the states. 	
	    if (strlen(argv[2+param]) == h->n) initstae_(q,h->n,argv[2+param],a);
	    // else, it means the user is specifying the stabilizers on the command line.
	    else initstae_from_stab_(q,h->n,argv[2+param],a);
          }

	}
	
	else if (argc==(5+param))  // if no instates or stabilizers specified
	{
	  initstae_(q,h->n,NULL,a);
	}
