...
p1, p2, p_meas = float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5])
...
# This is temporary just to have data for n = 6,9 for the NN.
...
if QEC_kind == 'surface17':
    output_folder = './MC_results/QECd3_surface17/traditional_sampler/'+error_model+'/'+str(n_rounds)+'/'+state+'/'
...
def run_several_QEC_fast(error_info, n_runs_total, init_state, QEC_kind, QEC_circ_list):
    ...
    for n_run in xrange(n_runs_total):
        ...
        if QEC_kind == 'surface17':
            ...
            carry_run, faulty_circs = wrapper.add_errors_general_sampler_new(
                                                QEC_circ_list_copy,
                                                error_info)
...
json_filename = str(p1) + '_' + str(p2) + '_' + str(p_meas) + '.json'   # The name of the file is the value of p_Igate, pwait, p_gate.  MGA 11/13/20.  MGA 2/1/21.
